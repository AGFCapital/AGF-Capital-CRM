...
const state = {
  leads: [], projects: [], profiles: [], settings: { bookingUrl: "", timezone: "America/Sao_Paulo", callDuration: 30, slotInterval: 15, messageTemplates: { ...defaultMessageTemplates } },
...
function schedulingMessage(lead) { return renderMessageTemplate(state.settings.messageTemplates.scheduling, { Nome: firstName(lead.contact), link: bookingLink() }); }
...
function settingsFromRows(rows) {
  ...
  return { bookingUrl: value.booking_url || defaultBookingUrl, timezone: value.timezone || "America/Sao_Paulo", callDuration: value.duration_minutes || 30, slotInterval: value.slot_minutes || 15, messageTemplates: readMessageTemplates(templateValue) };
}
...
async function saveSettings(values) {
  try {
    ...
    await reloadAndRender("Agenda, modelos de mensagens e notificações salvos.");
  } catch (error) {
