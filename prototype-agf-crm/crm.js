...
const state = {
  ...
  settingsOpen: false, projectModal: null, followUpLeadId: null, followUpNotificationTimer: null, lastFollowUpAlertKey: "",
...
function sourceClass(source) { return source === "Vagas" ? "vacancy" : source === "Manual" ? "manual" : "middle"; }
...
function mapRemoteLead(row) {
  ...
  return {
    ...
    source: row.source === "vacancy" ? "Vagas" : row.source === "manual_referral" ? "Manual" : "Middle market", stage: row.current_stage, score: row.total_score || 0,
...
function settingsFromRows(rows) {
  ...
  return { bookingUrl: value.booking_url || "", timezone: value.timezone || "America/Sao_Paulo", callDuration: value.duration_minutes || 30, slotInterval: value.slot_minutes || 15 };
}
