...
const defaultBookingUrl = "https://calendar.app.google/ohzMARRH81sZthT49";
...
const validTargets = {
  ...
  convite_enviado: ["conexao_aceita", "convite_expirado", "descartado"],
...
function messageFor(row, company, contact, signals) {
  ...
  return defaultMessage.replace("{Nome}", firstName(contact?.full_name)).replace("{trigger da empresa}", trigger);
}
function mapRemoteLead(row) {
  ...
  const booking = (row.calendar_bookings || []).find((item) => item.match_status !== "unmatched") || (row.calendar_bookings || [])[0];
  ...
  return {
    ...
    source: row.import_origin?.startsWith("Apollo CSV") ? "Apollo" : row.source === "vacancy" ? "Vagas" : row.source === "manual_referral" ? "Manual" : "Middle market", stage: row.current_stage, score: row.total_score || 0,
...
function normalizeCompanyName(value = "") { return String(value).normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/&/g, " e ").replace(/\b(s\/?a|ltda|limitada|eireli|me)\b/g, " ").replace(/[^a-z0-9]+/g, " ").trim().replace(/\s+/g, " "); }
function normalizeLinkedInUrl(value = "") { try { const url = new URL(String(value).trim()); const path = decodeURIComponent(url.pathname).replace(/\/+$/, "").toLowerCase(); return url.hostname.toLowerCase().includes("linkedin.com") && path.startsWith("/in/") ? `https://www.linkedin.com${path}` : ""; } catch { return ""; } }
function parseDelimitedCsv(text, delimiter = ";") {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    if ((char === "\n" || char === "\r") && !quoted) { if (char === "\r" && next === "\n") index += 1; row.push(value); if (row.some((cell) => cell !== "")) rows.push(row); row = []; value = ""; continue; }
    value += char;
  }
  row.push(value); if (row.some((cell) => cell !== "")) rows.push(row);
  const [headers = [], ...data] = rows;
  return data.map((cells, rowIndex) => Object.fromEntries(headers.map((header, columnIndex) => [header, cells[columnIndex] || ""]).concat([["__row", rowIndex + 2]])));
}
function apolloCompanySize(employeeCount) { const count = Number(employeeCount); return Number.isFinite(count) ? (count > 1000 ? "large" : count >= 201 ? "medium" : "small") : null; }
function prepareApolloImport(rows, fileName) {
  const errors = []; const records = []; const seenCompanies = new Set(); const seenContacts = new Set();
  rows.forEach((row) => {
    const cell = (column) => String(row[column] || "").trim();
    const companyName = cell("Company Name"); const fullName = `${cell("First Name")} ${cell("Last Name")}`.trim(); const linkedinUrl = normalizeLinkedInUrl(cell("Person Linkedin Url"));
    if (!companyName || !fullName || !linkedinUrl) { errors.push(`Linha ${row.__row}: empresa, nome ou LinkedIn ausente/inválido.`); return; }
    const normalizedCompany = normalizeCompanyName(companyName);
    if (seenCompanies.has(normalizedCompany)) { errors.push(`Linha ${row.__row}: empresa repetida no arquivo (${companyName}).`); return; }
    if (seenContacts.has(linkedinUrl)) { errors.push(`Linha ${row.__row}: contato repetido no arquivo (${fullName}).`); return; }
    seenCompanies.add(normalizedCompany); seenContacts.add(linkedinUrl);
    records.push({
      sourceRow: row.__row, companyName, normalizedCompany, companyLinkedinUrl: cell("Company Linkedin Url") || null,
      website: cell("Website") || null, industry: cell("Industry") || null, city: cell("Company City") || cell("City") || null,
      state: cell("Company State") || cell("State") || null, country: cell("Company Country") || cell("Country") || null,
      employees: Number.isFinite(Number(cell("# Employees"))) ? Number(cell("# Employees")) : null, companySize: apolloCompanySize(cell("# Employees")),
      annualRevenue: Number.isFinite(Number(cell("Annual Revenue"))) ? Number(cell("Annual Revenue")) : null,
      fullName, title: cell("Title") || null, linkedinUrl, apolloContactId: cell("Apollo Contact Id") || null, apolloAccountId: cell("Apollo Account Id") || null,
  ...
  return { fileName, total: rows.length, records, errors };
}
...
async function loadRemoteData() {
  const leadQuery = "/rest/v1/leads?select=id,current_stage,total_score,company_size_score,urgency_score,financial_moment_score,decision_maker_score,real_economy_bonus,source,import_origin,company_overview,contact_context,company:companies(id,name,headquarters_city,headquarters_state,real_economy),contact:contacts(full_name,title,linkedin_url,profile_gate_passed,profile_gate_reason,connection_count,location_country),lead_signals(summary,family,source_url,source_name,published_at,occurred_at,verified_at,verification_method),message_drafts(id,body,is_current),calendar_bookings(starts_at,status,meeting_url,match_status),lead_activities(summary,created_at),lead_follow_ups(id,due_at,note,status,completed_at)&order=created_at.desc";
  const [leadRows, projectRows, settingRows] = await Promise.all([
    supabaseRequest(leadQuery), supabaseRequest("/rest/v1/commercial_projects?select=*&order=updated_at.desc"), supabaseRequest("/rest/v1/app_settings?select=setting_key,value"),
  ]);
  state.leads = leadRows.map(mapRemoteLead); state.projects = projectRows.map(mapProject); state.settings = settingsFromRows(settingRows);
}
...
function card(lead) { const due = lead.followUps.find((item) => item.status === "open"); return `<article class="lead-card" draggable="true" data-lead="${lead.id}" tabindex="0"><div class="card-top"><span class="badge ${sourceClass(lead.source)}">${lead.source}</span><span class="score ${scoreClass(lead.score)}">${lead.score}/10</span></div><h3>${escapeHtml(lead.company)}</h3><p class="contact">${escapeHtml(lead.contact)}<span>${escapeHtml(lead.role)}</span></p>${lead.meeting ? `<p class="meeting">${lead.meeting}</p>` : ""}${due ? `<p class="followup-chip">Follow-up: ${humanDate(due.due_at)}</p>` : ""}<footer><span class="card-stage">${stageNames[lead.stage]}</span>${lead.linkedinUrl ? `<button class="card-linkedin" data-action="open-linkedin" data-lead="${lead.id}">LinkedIn ↗</button>` : ""}</footer></article>`; }
...
function operationPage() { const board = boardColumns.map((column) => { const items = state.leads.filter((lead) => column.stages.includes(lead.stage)); return `<section class="kanban-column" data-column="${column.key}" data-target="${stageDropTargets[column.key] || ""}"><header><h2>${column.label}</h2><span>${items.length}</span></header><div class="cards">${items.map(card).join("") || emptyStage(column.key)}</div></section>`; }).join(""); return appShell(`${header("OPERACAO COMERCIAL", "Base de clientes", "Arraste o card para concluir a proxima acao ou abra-o para revisar o contexto.", `<button class="button secondary" data-action="import-csv">Importar CSV</button><button class="button secondary" data-action="settings">Configurar agenda</button>`) }${metrics()}<div class="board-caption"><span>O Kanban e horizontal; cada coluna mantem sua propria rolagem de cards.</span><div><span class="status-dot"></span> LinkedIn operado manualmente</div></div><div class="kanban-scroll"><div class="kanban">${board}</div></div>${drawOverlays()}`); }
...
function importCsvModal() { const report = state.importModal; const preview = report.records?.slice(0, 4) || []; return `<div class="backdrop" data-action="close-import-csv"></div><section class="small-modal import-modal" role="dialog" aria-label="Importar contatos do Apollo"><header><div><p class="eyebrow">IMPORTACAO DE LEADS</p><h2>CSV filtrado do Apollo</h2><p>Todos os registros válidos entram em Base de clientes, em revisão manual.</p></div><button data-action="close-import-csv">×</button></header>${report.records ? `<div class="import-summary"><strong>${report.records.length} prontos para importar</strong><span>${report.total} linhas lidas · ${report.errors.length} linha(s) recusada(s)</span></div><ul class="import-preview">${preview.map((record) => `<li><b>${escapeHtml(record.companyName)}</b><span>${escapeHtml(record.fullName)} · ${escapeHtml(record.title || "Cargo ausente")}</span></li>`).join("")}</ul>${report.errors.length ? `<p class="warning-copy">${report.errors.slice(0, 3).map(escapeHtml).join(" ")}${report.errors.length > 3 ? " …" : ""}</p>` : ""}<footer><button class="button secondary" data-action="reset-import-csv">Escolher outro arquivo</button><button class="button primary" data-action="commit-import-csv">Importar ${report.records.length} leads</button></footer>` : `<form id="import-csv-form"><label>Arquivo CSV exportado do Apollo<input type="file" name="file" accept=".csv,text/csv" required></label><p class="muted-copy">O importador usa empresa, cargo, localização, porte, faturamento e URL do LinkedIn. E-mails, telefones e tecnologias não são armazenados.</p><footer><button type="button" class="button secondary" data-action="close-import-csv">Cancelar</button><button class="button primary" type="submit">Ler arquivo</button></footer></form>`}</section>`; }
function historyPage() { const rows = [...state.leads].sort((a, b) => a.company.localeCompare(b.company)); return appShell(`${header("HISTORICO", "Empresas e contatos", "Uma empresa mantem seu historico comercial, inclusive depois de encerrada.")}<section class="history-table"><div class="history-head"><span>Empresa e contato</span><span>Origem</span><span>Score</span><span>Etapa</span></div>${rows.map((lead) => `<button class="history-row" data-lead="${lead.id}"><div><strong>${escapeHtml(lead.company)}</strong><small>${escapeHtml(lead.contact)} | ${escapeHtml(lead.role)}</small></div><span>${lead.source}</span><span class="score ${scoreClass(lead.score)}">${lead.score}/10</span><span>${stageNames[lead.stage]}</span></button>`).join("")}</section>${drawOverlays()}`); }
...
function render() { if (state.remote.loading) { root.innerHTML = `<main class="loading-page">Carregando CRM AGF...</main>`; return; } if (state.remote.error || !state.remote.enabled) { root.innerHTML = `<main class="loading-page">O CRM precisa de uma configuracao Supabase valida. ${escapeHtml(state.remote.error || "Configuracao ausente.")}</main>`; return; } if (!state.remote.session?.access_token) { root.innerHTML = loginPage(); bindEvents(); return; } root.innerHTML = state.page === "agenda" ? agendaPage() : state.page === "followups" ? followUpsPage() : state.page === "projects" ? projectsPage() : state.page === "history" ? historyPage() : operationPage(); bindEvents(); scheduleFollowUpNotifications(); requestAnimationFrame(notifyDueFollowUps); }
...
function isActiveLeadStage(stage) { return !["concluido", "convite_expirado", "descartado"].includes(stage); }
function websiteDomain(value) { try { return new URL(value).hostname.replace(/^www\./, ""); } catch { return null; } }
async function findOne(path) { const rows = await supabaseRequest(path); return rows[0] || null; }
async function importApolloRecords(report) {
  const result = { imported: 0, duplicates: 0, failed: [] };
  for (const record of report.records) {
    try {
      let company = await findOne(`/rest/v1/companies?select=id&normalized_name=eq.${encodeURIComponent(record.normalizedCompany)}`);
      if (!company) {
        const rows = await supabaseRequest("/rest/v1/companies", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ name: record.companyName, normalized_name: record.normalizedCompany, linkedin_url: record.companyLinkedinUrl, website_domain: websiteDomain(record.website), industry: record.industry, headquarters_city: record.city, headquarters_state: record.state, employee_count: record.employees, company_size: record.companySize, revenue_proxy_min: record.annualRevenue, revenue_proxy_max: record.annualRevenue }) });
        company = rows[0];
      }
      const companyLeads = await supabaseRequest(`/rest/v1/leads?select=id,current_stage&company_id=eq.${encodeURIComponent(company.id)}`);
      if (companyLeads.some((lead) => isActiveLeadStage(lead.current_stage))) { result.duplicates += 1; continue; }
      let contact = await findOne(`/rest/v1/contacts?select=id&linkedin_url=eq.${encodeURIComponent(record.linkedinUrl)}`);
      if (contact) {
        const contactLeads = await supabaseRequest(`/rest/v1/leads?select=id,current_stage&contact_id=eq.${encodeURIComponent(contact.id)}`);
        if (contactLeads.some((lead) => isActiveLeadStage(lead.current_stage))) { result.duplicates += 1; continue; }
      } else {
        const rows = await supabaseRequest("/rest/v1/contacts", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ full_name: record.fullName, linkedin_url: record.linkedinUrl, title: record.title, location_country: record.country === "Brazil" ? "Brasil" : record.country, profile_gate_passed: false, profile_gate_reason: "Importado do Apollo; conexões do LinkedIn ainda não verificadas." }) });
        contact = rows[0];
      }
      await supabaseRequest("/rest/v1/leads", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ company_id: company.id, contact_id: contact.id, source: "manual_referral", current_stage: "revisao_manual", import_origin: record.importOrigin, company_overview: [record.industry, record.employees ? `${record.employees} funcionários` : null, record.annualRevenue ? `receita Apollo: ${record.annualRevenue}` : null].filter(Boolean).join(" | "), contact_context: record.title || null }) });
      result.imported += 1;
    } catch (error) { result.failed.push(`Linha ${record.sourceRow}: ${error.message || "erro no Supabase"}`); }
  }
  state.importModal = null;
  await reloadAndRender(`${result.imported} leads importados. ${result.duplicates} duplicata(s) não incluída(s).${result.failed.length ? ` ${result.failed.length} linha(s) falharam.` : ""}`);
  if (result.failed.length) console.warn("Falhas na importação Apollo:", result.failed);
}
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-action]").forEach((button) => button.addEventListener("click", () => { const action = button.dataset.action; const lead = leadById(button.dataset.lead); if (action === "close-detail") { state.selectedId = null; render(); } if (action === "close-project-detail") { state.selectedProjectId = null; render(); } if (action === "settings") { state.settingsOpen = true; render(); } if (action === "close-settings") { state.settingsOpen = false; render(); } if (action === "logout") { writeSession(null); state.selectedId = null; state.selectedProjectId = null; render(); } if (action === "open-linkedin" && lead?.linkedinUrl) window.open(lead.linkedinUrl, "_blank", "noopener,noreferrer"); if (action === "open-linkedin" && !lead?.linkedinUrl) toast("Este contato ainda nao possui URL do LinkedIn."); if (action === "copy-invite") void copyText(schedulingMessage(lead), "Nota de convite copiada."); if (action === "copy-scheduling") { void recordActivity(lead.id, "booking_link_copied", "Mensagem de agendamento copiada manualmente.").then(() => copyText(schedulingMessage(lead), "Mensagem de agendamento copiada.")); } if (action === "copy-message") { const text = document.querySelector("#message-draft")?.value || lead.message; void copyText(text, "Mensagem copiada."); } if (action === "save-message") void saveMessage(lead); if (action === "new-followup") { state.followUpLeadId = lead.id; render(); } if (action === "close-followup-modal") { state.followUpLeadId = null; render(); } if (action === "complete-followup") void completeFollowUp(button.dataset.followup); if (action === "new-project") { state.projectModal = ""; render(); } if (action === "project-from-lead") { state.projectModal = lead.id; render(); } if (action === "close-project-modal") { state.projectModal = null; render(); } }));
  ...
  document.querySelectorAll("[data-action=commit-import-csv]").forEach((button) => button.addEventListener("click", () => { if (state.importModal?.records?.length) void importApolloRecords(state.importModal); }));
  ...
  document.querySelector("#import-csv-form")?.addEventListener("submit", async (event) => { event.preventDefault(); const file = event.currentTarget.elements.file.files[0]; if (!file) return; try { state.importModal = prepareApolloImport(parseDelimitedCsv(await file.text()), file.name); render(); } catch (error) { toast(error.message || "Não foi possível ler o CSV."); } });
}
