...
const state = {
  leads: [], projects: [], profiles: [], settings: { timezone: "America/Sao_Paulo", callDuration: 30, slotInterval: 15, messageTemplates: { ...defaultMessageTemplates } },
...
function schedulingProfile(lead) { return responsibleProfile(lead?.schedulingProfileId || lead?.responsibleId); }
function bookingLink(lead) { return lead?.schedulingBookingUrl || schedulingProfile(lead)?.booking_url || ""; }
function inviteNoteMessage(lead) { return renderMessageTemplate(state.settings.messageTemplates.inviteNote, { Nome: firstName(lead.contact), Empresa: lead.company || "sua empresa" }); }
function schedulingMessage(lead) { return renderMessageTemplate(state.settings.messageTemplates.scheduling, { Nome: firstName(lead.contact), link: bookingLink(lead) || "[link da agenda ainda nao configurado]" }); }
function bookingThanksMessage(lead) { return renderMessageTemplate(state.settings.messageTemplates.bookingThanks, { Nome: firstName(lead.contact), data_hora: lead.meeting || "o horário escolhido" }); }
function standardMessageFor(company, contact) { return renderMessageTemplate(state.settings.messageTemplates.linkedinMessage, { Nome: firstName(contact?.full_name), Empresa: company?.name || "sua empresa" }); }

async function fetchConfiguration() {
  // Relative to this module's own URL, not the page's root — so this keeps
  // resolving correctly when mounted under a subpath (e.g. proxied at
  // agfcapital.com.br/admin/crm/), not just when served from a domain root.
  const response = await fetch(new URL("./api/config", import.meta.url), { cache: "no-store" });
...
function mapRemoteLead(row) {
  ...
  return {
    ...
    schedulingProfileId: row.scheduling_profile_id || null,
    schedulingBookingUrl: row.scheduling_booking_url || "",
    schedulingStartedAt: row.scheduling_started_at || null,
...
function settingsFromRows(rows) {
  ...
  const templateValue = rows.find((row) => row.setting_key === "message_templates")?.value || {};
  return { timezone: value.timezone || "America/Sao_Paulo", callDuration: value.duration_minutes || 30, slotInterval: value.slot_minutes || 15, messageTemplates: readMessageTemplates(templateValue) };
}
...
async function loadRemoteData() {
  const leadQuery = "/rest/v1/leads?select=id,current_stage,total_score,company_size_score,urgency_score,financial_moment_score,decision_maker_score,real_economy_bonus,source,import_origin,company_overview,contact_context,organization_label,responsible_id,scheduling_profile_id,scheduling_booking_url,scheduling_started_at,updated_at,stage_entered_at,company:companies(id,name,industry,headquarters_city,headquarters_state,real_economy),contact:contacts(full_name,title,linkedin_url,profile_gate_passed,profile_gate_reason,connection_count,location_country),lead_signals(summary,family,source_url,source_name,published_at,occurred_at,verified_at,verification_method),message_drafts(id,body,is_current,updated_at),calendar_bookings(provider_event_id,provider_created_at,starts_at,status,meeting_url,match_status,host_profile_id,source_calendar_id,created_at,updated_at),lead_activities(summary,created_at),lead_follow_ups(id,due_at,note,status,completed_at,assigned_to)&order=created_at.desc";
  const [leadRows, projectRows, settingRows, leadPool, profileRows] = await Promise.all([
    ...
    supabaseRequest("/rest/v1/profiles?select=id,full_name,role,notification_email,follow_up_email_enabled,booking_url,calendar_id,calendar_enabled&order=full_name.asc"),
  ]);
...
function followUpModal(target, entity) { const title = target.type === "project" ? entity.name : entity.company; const owner = responsibleProfile(entity.responsibleId); return `<div class="backdrop" data-action="close-followup-modal"></div><section class="small-modal" role="dialog" aria-label="Adicionar follow-up"><header><div><p class="eyebrow">FOLLOW-UP · ${target.type === "project" ? "PROJETO" : "LEAD"}</p><h2>${escapeHtml(title)}</h2><p>${owner ? `O lembrete sera enviado somente para ${escapeHtml(owner.full_name || owner.notification_email || "o responsavel do card")}.` : "Defina um responsavel no card antes de criar o lembrete."}</p></div><button data-action="close-followup-modal">×</button></header><form id="followup-form"><input name="targetType" type="hidden" value="${target.type}"><input name="targetId" type="hidden" value="${target.id}"><label>Data e hora<input name="dueAt" type="datetime-local" required></label><label>Proxima acao<textarea name="note" required placeholder="Ex.: retomar conversa sobre o fechamento mensal"></textarea></label><footer><button type="button" class="button secondary" data-action="close-followup-modal">Cancelar</button><button class="button primary" type="submit" ${owner ? "" : "disabled"}>Criar lembrete</button></footer></form></section>`; }
...
async function completeLeadDrop(leadId, targetStage) {
  ...
  if (targetStage === "agendamento") {
    const owner = responsibleProfile(lead.responsibleId);
    if (!owner) {
      toast("Defina um responsável no card antes de iniciar o agendamento.");
      return;
    }
    if (!owner.calendar_enabled || !owner.booking_url) {
      toast(`${profileLabel(owner)} precisa configurar o próprio link de agenda.`);
      return;
    }
  }
  const transitions = {
...
async function createFollowUp(values) {
  ...
  const entity = target.type === "project" ? projectById(target.id) : leadById(target.id);
  const owner = responsibleProfile(entity?.responsibleId);
  if (!owner) {
    toast("Defina um responsavel no card antes de criar o follow-up.");
    return;
  }
  try {
    ...
    await reloadAndRender(`Follow-up criado para ${owner.full_name || owner.notification_email}.`);
  } catch (error) {
...
async function saveSettings(values) {
  try {
    const messageTemplates = Object.fromEntries(messageTemplateFields.map((field) => [field.key, values[field.key]]));
    const templateError = validateMessageTemplates(messageTemplates);
    if (templateError) { toast(templateError); return; }
    await Promise.all([
      supabaseRequest("/rest/v1/app_settings?setting_key=eq.message_templates", { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ value: serializeMessageTemplates(messageTemplates) }) }),
      supabaseRequest(`/rest/v1/profiles?id=eq.${encodeURIComponent(state.remote.session.user.id)}`, {
        method: "PATCH",
        headers: { Prefer: "return=minimal" },
        body: JSON.stringify({
          booking_url: values.bookingUrl.trim(),
    ...
    await reloadAndRender("Minha agenda, modelos de mensagens e notificações foram salvos.");
  } catch (error) {
