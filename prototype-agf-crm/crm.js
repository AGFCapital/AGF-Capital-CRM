...
const state = {
  ...
  settingsOpen: false, projectModal: null, manualLeadModal: false, followUpLeadId: null, importModal: null, followUpNotificationTimer: null,
...
function notificationCenter() {
  ...
  const alerts = notificationItems();
  return `<aside class="notification-center" aria-label="Central de notificacoes"><header><div><p class="eyebrow">LEMBRETES</p><h2>Notificacoes</h2></div><button data-action="toggle-notifications" aria-label="Fechar notificacoes">×</button></header><div class="notification-tabs"><button class="${state.notificationView === "mine" ? "active" : ""}" data-notification-view="mine">Minhas</button><button class="${state.notificationView === "team" ? "active" : ""}" data-notification-view="team">Equipe</button></div>${alerts.length ? `<div class="notification-list">${alerts.map((item) => `<button data-lead="${item.lead.id}"><span class="${item.overdue ? "urgent" : ""}">${item.overdue ? "Follow-up vencido" : "Proximas 24 horas"}</span><strong>${escapeHtml(item.lead.company)}</strong><small>${humanDate(item.due_at)} · ${escapeHtml(item.note)}</small><em>${escapeHtml(item.assignedName)}</em></button>`).join("")}</div><button class="text-link notification-footer" data-page="followups">Ver todos os follow-ups</button>` : `<p class="empty-state">Nenhum follow-up vencido ou previsto para as proximas 24 horas nesta visualizacao.</p>`}</aside>`;
}
function appShell(content) { return `<div class="crm-shell"><aside class="sidebar"><div class="brand"><div class="brand-mark">A</div><div><strong>AGF</strong><span>Capital</span></div></div><nav><button data-page="dashboard" class="nav-item ${state.page === "dashboard" ? "active" : ""}"><span>01</span> Visao geral</button><button data-page="operation" class="nav-item ${state.page === "operation" ? "active" : ""}"><span>02</span> Base de clientes</button><button data-page="followups" class="nav-item ${state.page === "followups" ? "active" : ""}"><span>03</span> Follow-ups <b>${state.leads.flatMap((lead) => lead.followUps).filter((item) => item.status === "open").length}</b></button><button data-page="agenda" class="nav-item ${state.page === "agenda" ? "active" : ""}"><span>04</span> Agendamentos <b>${state.leads.filter((lead) => lead.stage === "call_marcada").length}</b></button><button data-page="projects" class="nav-item ${state.page === "projects" ? "active" : ""}"><span>05</span> Projetos <b>${state.projects.filter((project) => !["ganho", "perdido"].includes(project.currentStage)).length}</b></button><button data-page="history" class="nav-item ${state.page === "history" ? "active" : ""}"><span>06</span> Base completa</button></nav><div class="sidebar-bottom"><button data-action="settings" class="settings-button">Configuracoes</button><button data-action="logout" class="settings-button">Sair</button><div class="operator"><div class="avatar">${operatorName().split(" ").map((part) => part[0]).join("").slice(0, 2)}</div><div><strong>${operatorName()}</strong><span>Base compartilhada</span></div></div></div></aside><section class="workspace">${workspaceUtility()}${notificationCenter()}${content}</section></div>`; }
...
function followUpsPage() { const items = state.leads.flatMap((lead) => lead.followUps.filter((item) => item.status === "open").map((item) => ({ ...item, lead }))).sort((a, b) => new Date(a.due_at) - new Date(b.due_at)); return appShell(`${header("FOLLOW-UPS", "Proximas acoes", "Todos veem a fila da equipe; o responsavel e o usuario que criou cada follow-up.")}<section class="agenda-list">${items.length ? items.map((item) => `<button class="agenda-row" data-lead="${item.lead.id}"><span class="agenda-time">${humanDate(item.due_at)}</span><div><strong>${escapeHtml(item.lead.company)}</strong><small>${escapeHtml(item.note)} · ${escapeHtml(item.assignedName)}</small></div><span>Ver card</span></button>`).join("") : `<div class="empty-page">Nenhum follow-up aberto.</div>`}</section>${drawOverlays()}`); }
function projectCard(project) { return `<article class="project-card" draggable="true" data-project="${project.id}" tabindex="0"><span class="project-stage">${projectStageNames[project.currentStage]}</span><h3>${escapeHtml(project.name)}</h3><p class="project-company">${escapeHtml(project.company_name)}</p>${project.estimated_value != null ? `<strong class="project-value">${formatBrlCurrency(project.estimated_value)}</strong>` : ""}<div class="project-owner"><span>Responsavel</span><strong>${escapeHtml(project.responsible_name)}</strong></div><footer><div><small>Proxima acao</small><strong>${project.next_action_at ? `${humanDate(project.next_action_at)} · ${escapeHtml(project.next_action || "Acao pendente")}` : "Nao definida"}</strong></div><span>Abrir</span></footer></article>`; }
...
function drawOverlays() { const selectedProject = projectById(state.selectedProjectId); return `${state.selectedId ? operationalLeadDrawer(leadById(state.selectedId)) : ""}${selectedProject ? enhanceProjectValueField(projectDrawer(selectedProject), selectedProject.estimated_value) : ""}${state.settingsOpen ? settingsModal() : ""}${state.projectModal !== null ? enhanceProjectValueField(projectModal(state.projectModal ? leadById(state.projectModal) : null)) : ""}${state.manualLeadModal ? manualLeadModal() : ""}${state.followUpLeadId ? followUpModal(leadById(state.followUpLeadId)) : ""}${state.importModal ? importCsvModal() : ""}`; }
...
async function refreshRemoteData() {
  const hasOpenInteraction = state.selectedId || state.selectedProjectId || state.settingsOpen || state.projectModal !== null || state.manualLeadModal || state.followUpLeadId || state.importModal || state.draggedId || state.draggedProjectId;
...
async function createFollowUp(values) { try { await supabaseRequest("/rest/v1/lead_follow_ups", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ lead_id: values.leadId, due_at: new Date(values.dueAt).toISOString(), note: values.note, created_by: state.remote.session.user.id, assigned_to: state.remote.session.user.id }) }); await recordActivity(values.leadId, "follow_up_created", `Follow-up agendado para ${humanDate(new Date(values.dueAt).toISOString())}.`); state.followUpLeadId = null; await reloadAndRender("Follow-up criado para voce."); } catch (error) { toast(error.message || "Nao foi possivel criar o follow-up."); } }
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-project]").forEach((node) => { node.addEventListener("click", () => { state.selectedId = null; state.selectedProjectId = node.dataset.project; render(); }); node.addEventListener("dragstart", () => { state.draggedProjectId = node.dataset.project; node.classList.add("dragging"); }); node.addEventListener("dragend", () => { state.draggedProjectId = null; node.classList.remove("dragging"); }); });
